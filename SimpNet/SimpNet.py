...
from keras.layers import Input, Conv2D, MaxPooling2D, GlobalMaxPooling2D
...
def block(name, x, F, K, act, init, pool=False):
    out = Conv2D(filters=F, kernel_size=K, strides=1, use_bias=True,
	padding='same', kernel_initializer=init, name=name)(x)
    out = BatchNormalization(momentum=0.95, center=True, scale=True,
	name=name+'_batchnorm')(out)
...
def SimpNet_CIFAR10_9M(H, W, C, num_classes, act, drop_rate=0.25, reg=0):
    ...
    # Block 1
    block1 = Conv2D(filters=128, kernel_size=3, strides=1, use_bias=True, padding='same', kernel_initializer=glorot_normal(), input_shape=[H,W,C], name='block1')(input_img)
    block1 = BatchNormalization(momentum=0.95, center=True, scale=True, name='block1_batchnorm')(block1)
    ...
    # Final Classifier
    softmax = Dense(num_classes, activation='softmax', name='softmax', kernel_initializer=glorot_normal(), bias_initializer=Zeros())(block13)
...
def SimpNet_CIFAR10_5M(H, W, C, num_classes, act, drop_rate=0.25, reg=0):
    ...
    # Block 1
    block1 = Conv2D(filters=66, kernel_size=3, strides=1, use_bias=True, padding='same', kernel_initializer=glorot_normal(), input_shape=[H,W,C], name='block1')(input_img)
    block1 = BatchNormalization(momentum=0.95, center=True, scale=True, name='block1_batchnorm')(block1)
    block1 = Activation(act, name='block1_act')(block1)
    block1 = Dropout(0.2, name='block1_dropout')(block1)
